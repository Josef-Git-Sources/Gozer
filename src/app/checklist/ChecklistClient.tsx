...
import { CheckCircle, Circle, LogIn, Cloud, CloudOff } from 'lucide-react'
...
export function ChecklistClient({ groups }: { groups: Group[] }) {
  ...
  // טען סימונים מה-DB כשמשתמש מחובר
  useEffect(() => {
  ...
  async function toggle(id: string) {
    ...
    if (session) {
      ...
      await fetch('/api/user/checklist', {
        ...
        body: JSON.stringify({ itemId: id, completed: nowCompleted }),
      }).catch(() => {})
      setSyncing(false)
    }
  }
  ...
  if (!loaded) return (
    <div className="max-w-3xl mx-auto px-4 py-20 text-center text-gray-400">טוען...</div>
  )
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fade-in">
      ...
      {/* סטטוס סנכרון */}
      {session && (
        <div className="flex items-center justify-end gap-1.5 mb-4 text-xs text-gray-400">
          {syncing
            ? <><Cloud size={13} className="animate-pulse" /> שומר...</>
            : <><Cloud size={13} className="text-green-mid" /> מסונכרן</>}
        </div>
      )}
